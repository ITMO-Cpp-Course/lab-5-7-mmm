...
class InvertedIndex
{
  private:
    ...
    std::unordered_map<std::string, std::unordered_map<size_t, size_t>> index_; //внешний - слово, внутренний - {id, частота вхождения}
    std::unordered_map<size_t, std::vector<std::string>> doc_to_words_; //ключ - id, значение - вектор слов в порядке появления, хранит список слов (токенов) для каждого элемента
  ...
  public:
    ...
    size_t get_word_count(std::string word, size_t doc_id) const; //сколько раз определенное слово встречается в документе
